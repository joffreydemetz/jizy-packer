...
export function mirrorExampleFiles(targetDir, replacements) {
    ...
    files.forEach(file => {
        ...
        if (content) {
            content = content.toString().replace(/%MODULE_ALIAS%/g, moduleAlias);
            content = content.toString().replace(/%MODULE_NAME%/g, moduleName);
            content = content.toString().replace(/%GIT_ACCOUNT%/g, gitAccount);
            content = content.toString().replace(/%GIT_REPO%/g, gitRepo);
            content = content.toString().replace(/%GIT_MODULE%/g, gitModule);
            content = content.toString().replace(/%AUTHOR_NAME%/g, authorName);
            content = content.toString().replace(/%AUTHOR_EMAIL%/g, authorEmail);
            content = content.toString().replace(/%AUTHOR_WEBSITE%/g, authorWebsite);
        }
